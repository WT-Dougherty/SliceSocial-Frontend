...
import {
  apiGetPostPhoto,
  apiGetProPic,
} from '../services/api/endpoints/photos';
    ...
    useCallback(() => {
      async function fetchScreen() {
        ...
        try {
          ...
          Alert.alert(
            'Something went wrong while fetching profile info',
        ...
        try {
          ...
          console.log('Error:', err instanceof Error ? err.message : err);
          Alert.alert(
            'Something went wrong while fetching pro-pic',
            err instanceof Error ? err.message : 'Unknown error',
          );
        }
        ...
        try {
          ...
          const urls = await Promise.all(
            posts.map(post => apiGetPostPhoto(post.postID)),
          );
      ...
      <View style={styles.postsGrid}>
        {posts.map((post, index) => (
          <TouchableOpacity
            ...
            onPress={() =>
              rootNavigationRef.navigate('PostViewer', {
                post: post,
                url: postsURL[index],
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={styles.postsSection}>{renderPostGrid()}</View>
      </ScrollView>
    </View>
  );
}
